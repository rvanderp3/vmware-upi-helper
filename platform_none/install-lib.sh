...
function startMasters() {
    ...
    do    
        ...
        ROLE=master
        createAndConfigureVM $VM_NAME master $CPU_CORES $MEMORY_MB $GOVC_DATASTORE $DISK_SIZE "$SUBNET_PREFIX.$(( CONTROL_PLANE_START_IP + MASTER_INDEX ))::$SUBNET_GATEWAY:$SUBNET_NETMASK:$VM_NAME::none:$INFRA_VM_IP" $BASE_TEMPLATE &
        let MASTER_INDEX++
    done
}
